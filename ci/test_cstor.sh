...
setup_test_env() {
	...
	sed -i "s|LUN0 Storage.*|LUN0 Storage \/tmp\/istgtvol\/volume $size 32k|g" $CONF_FILE
...
start_replica() {
	...
	docker exec $replica_id $ZFS create -V1g -b 4k -o openebs.io:targetip="$1" replica_pool/volume
	echo "$replica_id"
}
